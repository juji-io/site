...
In the search of more efficient and objective computational approaches, Juji has invented a new model that combines big data analytics with [Item Response Theory](https://en.wikipedia.org/wiki/Item_response_theory) to infer individual differences from user behavior. Juji now uses this approach to infer [Big 5 personality scores](https://en.wikipedia.org/wiki/Big_Five_personality_traits) from text on 35 dimensions without a personality test.