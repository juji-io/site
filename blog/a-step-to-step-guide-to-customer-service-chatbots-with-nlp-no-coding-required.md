...
It is worth mentioning an often used "NLP" technique supported by some chatbot platforms: **keyword spotting**, which retrieves potential answers based on certain keywords mentioned in a user's question.  Unfortunately,  **keyword-based processing is often inadequate for a customer service chatbot** because it cannot distinguish the semantic differences between customer questions containing the same keywords, such as "*Can I get a refund for the course I paid for*" vs. "*When will I receive my refund*". Moreover, users would not be happy if they ask the latter question but get the answer to the former question, or vice versa. Here is a [short Youtube video](https://youtu.be/gBWab3yOsco) showing the difference between understanding the keywords vs. the semantics of user input.
